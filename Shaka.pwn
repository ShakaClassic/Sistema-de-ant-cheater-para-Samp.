...
#define FILTERSCRIPT
#define Shaka 0xFFFFFFAA 
#define virgem 0xFFFFFF00
new newScore[MAX_PLAYERS];
new killTimestamps[MAX_PLAYERS][11];
new killIndex[MAX_PLAYERS];

// Função para salvar informações do jogador em um arquivo
stock SavePlayerInfo(playerid, reason[])
{
    new file[256];
    new pname[MAX_PLAYER_NAME];
    GetPlayerName(playerid, pname, sizeof(pname));
    format(file, sizeof(file), "players/%s_%d.ini", pname, playerid); // Mudança da extensão para .ini
    ...
    // FPS, Taxa de acertos, Armas na mão e Dano por segundo podem necessitar de sistemas adicionais para serem registrados corretamente
}
...
{
    SendClientMessage(playerid, Shaka, "Iniciando modulos do ShakaByte-BOT");
    SendClientMessage(playerid, virgem, "Modulo V1 iniciado com sucesso!");
    SendClientMessage(playerid, virgem, "[ShakaByte-INFO] Todos os modulos V1 Iniciados com sucesso!");

    // Verificação básica de VPN/Proxy
    new ip[16];
...
{
    if (!IsPlayerAdmin(playerid)) // Verifica se o jogador não é administrador
    {
        new Float:health;
        GetPlayerHealth(playerid, health);
        if (health > 400.0)
        {
            KickPlayer(playerid, "ter vida acima do permitido");
        }

        // Verificação de Velocidade
        new Float:velX, Float:velY, Float:velZ;
        GetPlayerVelocity(playerid, velX, velY, velZ);
        if (floatabs(velX) > 400.0 || floatabs(velY) > 400.0 || floatabs(velZ) > 400.0) // Limite de velocidade
        {
            KickPlayer(playerid, "excesso de velocidade");
        }

        // Verificação de Altura
        new Float:pos[3];
        GetPlayerPos(playerid, pos[0], pos[1], pos[2]);
        if (pos[2] > 2000.0 || pos[2] < -50.0) // Limite de altura
        {
            KickPlayer(playerid, "estar em uma altura inválida");
        }

        // Verificação de troca de Score
        new score = GetPlayerScore(playerid);
        if (score > newScore[playerid] + 10) // Mudança repentina de score
        {
            KickPlayer(playerid, "alteração suspeita de score");
        }
        newScore[playerid] = score;

        // Verificação de mortes em 15 segundos
...
// Verificação de Arma e Dano causado
public OnPlayerGiveDamage(playerid, damagedid, Float:amount, weaponid, bodypart)
{
    if (!IsPlayerAdmin(playerid)) // Verifica se o jogador não é administrador
    {
        if (weaponid < 0 || weaponid > 46) // Intervalo de IDs de armas válidos
        {
            KickPlayer(playerid, "usar uma arma inválida");
        }

        // Verificação de Dano causado
        if (amount > 100.0)
        {
            KickPlayer(playerid, "causar dano excessivo");
        }
    }
    return 1;
}
...
// Função para kickar jogadores e salvar informações
